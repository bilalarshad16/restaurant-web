...
const CategoryForm = ({add, onClose, item, view, edit}) => {
  ...
  const getProducts = async () => {
    ...
    if (response && response.data){
      setProductData(response.data.filter(o=>!o.category).map(product => ({...product, key: product.id, label:product.name, value: product.id}))
      );
    }
  }
  ...
  return (
    <Form
      ...
      initialValues={{
        variant: componentVariant,
       name:item?.name,
       productIds:item?.products.map(product => ({...product, key: product.id, label:product.name, value: product.id}))
      }}
      onFinish={onFinish}
